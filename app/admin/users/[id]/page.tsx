  ...
  return (
    <AdminLayout>
      <div className="mb-8">
        ...
        <p className="text-gray-400">Manage and view details for {user.email}</p>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              ...
              <div className="mt-4 md:mt-0">
                <div className="flex space-x-2">
                  <button className="px-3 py-1.5 bg-gray-700 text-white rounded-md hover:bg-gray-600 transition-colors text-sm">
                    Send Message
                  </button>
                  <button
                    onClick={() => setShowEditModal(true)}
                    className="px-3 py-1.5 bg-blue-600 text-white rounded-md hover:bg-blue-500 transition-colors text-sm"
                  >
                    Edit Profile
                  </button>
                </div>
      ...
      <div className="bg-gray-800 rounded-lg shadow overflow-hidden">
              ...
              <div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-gray-200">{user.first_name} {user.last_name}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center">
                      ...
                      <p className="text-gray-200">{user.phone || 'N/A'}</p>
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-400">Account Type</label>
                    <p className="mt-1 text-gray-200">{user.account_type} ({user.tier})</p>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-400">Referral Code</label>
                    <p className="mt-1 text-gray-200">{user.referral_code || 'N/A'}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  {user.account_suspended && user.account_suspension_reason && (
                    <div>
                      <label className="block text-sm font-medium text-gray-400">Suspension Reason</label>
                      <p className="mt-1 text-gray-200">{user.account_suspension_reason}</p>
                    </div>
                  )}

                  <div>
                    <label className="block text-sm font-medium text-gray-400">Consent Preferences</label>
                    <div className="mt-1 space-y-2">
                      <div className="flex items-center">
                        ...
                        />
                        <label className="ml-2 text-sm text-gray-300">Marketing Communications</label>
                      </div>
                      <div className="flex items-center">
                        <input
                          type="checkbox"
                          ...
                          className="h-4 w-4 text-blue-600 rounded focus:ring-blue-500 bg-gray-700 border-gray-600"
                        />
                        <label className="ml-2 text-sm text-gray-300">Newsletter</label>
                      </div>
                      <div className="flex items-center">
                        <input
                          type="checkbox"
                          ...
                          className="h-4 w-4 text-blue-600 rounded focus:ring-blue-500 bg-gray-700 border-gray-600"
                        />
                        <label className="ml-2 text-sm text-gray-300">Privacy Policy</label>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center">
                      ...
                      <p className="text-gray-200">{formatDate(user.customer_since)}</p>
                    </div>
                  </div>
                    ...
                    {orders.map((order) => (
                      <tr key={order.id} className="hover:bg-gray-750">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <a href={`/admin/orders/${order.id}`} className="text-blue-400 hover:text-blue-300">
                            View
                          </a>
                        </td>
                      </tr>
        ...
        {activeTab === 'addresses' && (
          <div className="p-6">
            ...
            {addresses.length === 0 ? (
              <p className="text-gray-400">No addresses found for this user.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {addresses.map((address) => (
                  <div key={address.id} className="border border-gray-700 rounded-lg p-4">
        ...
        {activeTab === 'reviews' && (
          <div className="p-6">
            ...
            {reviews.length === 0 ? (
              <p className="text-gray-400">No reviews found for this user.</p>
            ) : (
              <div className="space-y-6">
                {reviews.map((review) => (
                  <div key={review.id} className="border border-gray-700 rounded-lg p-4">
                    ...
                    {review.title && (
                      <p className="mt-2 font-medium text-gray-200">{review.title}</p>
                    )}

                    {review.review && (
                      <p className="mt-1 text-gray-300">{review.review}</p>
                    )}
        ...
        {activeTab === 'wishlist' && (
          <div className="p-6">
            ...
            {wishlist.length === 0 ? (
              <p className="text-gray-400">No wishlist items found for this user.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {wishlist.map((item) => (
                  <div key={item.id} className="border border-gray-700 rounded-lg p-4 flex">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-400 mt-1">{formatCurrency(item.price)}</p>
                      <p className="text-xs text-gray-500 mt-1">Added: {formatDate(item.added_at)}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'cart' && (
          <div className="p-6">
            ...
            {cart.length === 0 ? (
              <p className="text-gray-400">No cart items found for this user.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-700">
                  <thead className="bg-gray-750">
